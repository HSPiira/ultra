...
def plan_list(*, filters: dict = None):
    ...
    if filters.get("query"):
        q = filters["query"]
        qs = qs.filter(Q(plan_name__icontains=q) | Q(description__icontains=q))
...
def plan_list_needing_attention():
    ...
    # Plans with no usage
    unused_plans = Plan.objects.filter(is_deleted=False, schemeitem__isnull=True)

    # Plans with missing descriptions
    plans_missing_description = Plan.objects.filter(is_deleted=False).filter(
        Q(description__isnull=True) | Q(description="")
    )

    # Combine all
    all_attention_needed = suspended_plans.union(
        unused_plans, plans_missing_description
    )
...
def plan_data_integrity_check():
    ...
    plans_missing_data = Plan.objects.filter(is_deleted=False).filter(
