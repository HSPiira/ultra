    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["company", "card_number"], name="uniq_company_card_number"
            )
        ]
    ...
    def clean(self):
        ...
        if self.relationship != RelationshipChoices.SELF:
            if not self.parent:
                ...
                if (
                    self.parent.company_id != self.company_id
                    or self.parent.scheme_id != self.scheme_id
                ):
                    errors["parent"] = (
                        "Parent must belong to the same company and scheme."
                    )
