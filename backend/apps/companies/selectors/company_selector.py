...
def company_statistics_get():
    ...
    industry_stats = (
        ...
        .values("industry__industry_name")
...
def company_list_with_recent_activity(*, days: int = 30):
    ...
    return (
        ...
        .filter(is_deleted=False, schemes__created_at__gte=cutoff_date)
        .distinct()
    )
...
def company_health_score_get(*, company_id: str):
    ...
    recent_activity = (
        company_list_with_recent_activity(days=30).filter(id=company_id).exists()
    )
    if recent_activity:
        score += 10
...
def company_list_needing_attention():
    ...
    companies_no_schemes = Company.objects.filter(
        is_deleted=False, schemes__isnull=True
    )
    ...
    companies_missing_contact = Company.objects.filter(is_deleted=False).filter(
    ...
    all_attention_needed = companies_no_schemes.union(
        companies_suspended, companies_missing_contact
    )
...
def company_data_integrity_check():
    ...
    companies_missing_data = Company.objects.filter(is_deleted=False).filter(
        Q(company_name__isnull=True)
        | Q(company_name="")
        | Q(contact_person__isnull=True)
        | Q(contact_person="")
        | Q(email__isnull=True)
        | Q(email="")
        | Q(phone_number__isnull=True)
        ...
        issues.append(
            {
                ...
                "companies": list(
                    companies_missing_data.values_list("id", "company_name")
                ),
            }
        )
