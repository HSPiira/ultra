...
from apps.providers.api.serializers import HospitalSerializer, DoctorSerializer
...
class ClaimDetailSerializer(BaseSerializer):
    class Meta(BaseSerializer.Meta):
        ...
        fields = BaseSerializer.Meta.fields + [
            'claim', 'item_description', 'quantity', 'unit_price', 'total_amount'
        ]
        extra_kwargs = {
            'claim': {'write_only': True, 'required': False}
        }
...
class ClaimSerializer(BaseSerializer):
    ...
    class Meta(BaseSerializer.Meta):
        model = Claim
        fields = BaseSerializer.Meta.fields + [
            'member', 'hospital', 'doctor', 'service_date', 'claim_status', 'invoice_number',
            'hospital_detail', 'doctor_detail', 'details', 'payments'
        ]
