...
def industry_statistics_get():
    ...
    stats = Industry.objects.filter(is_deleted=False).aggregate(
        ...
        inactive_industries=Count('id', filter=Q(status=BusinessStatusChoices.INACTIVE)),
        suspended_industries=Count('id', filter=Q(status=BusinessStatusChoices.SUSPENDED))
    )
    
    # Add company distribution
    industry_company_stats = Industry.objects.filter(
        is_deleted=False
    ).annotate(
        company_count=Count('companies', filter=Q(companies__is_deleted=False))
    ).values(
        'id', 'industry_name', 'company_count'
    ).order_by('-company_count')
    
    stats['by_company_count'] = list(industry_company_stats)
    
    # Add top industries by company count
    top_industries = Industry.objects.filter(
        is_deleted=False
    ).annotate(
        company_count=Count('companies', filter=Q(companies__is_deleted=False))
...
def industry_list_needing_attention():
    ...
    industries_missing_description = Industry.objects.filter(
        is_deleted=False
    ).filter(
    ...
    all_attention_needed = industries_no_companies.union(
        industries_suspended,
        industries_missing_description
    )
...
def industry_choices_get():
    ...
    """
    return list(Industry.objects.filter(
        is_deleted=False,
        status=BusinessStatusChoices.ACTIVE
    ).values_list('id', 'industry_name').order_by('industry_name'))
...
def industry_data_integrity_check():
    ...
    industries_missing_data = Industry.objects.filter(
        is_deleted=False
    ).filter(
    ...
    if industries_missing_data.exists():
        issues.append({
            ...
            'industries': list(industries_missing_data.values_list('id', 'industry_name'))
        })
    
    # Check for duplicate names
    duplicate_names = Industry.objects.filter(
        is_deleted=False
    ...
    if industries_no_companies.exists():
        issues.append({
            ...
            'industries': list(industries_no_companies.values_list('id', 'industry_name'))
        })
