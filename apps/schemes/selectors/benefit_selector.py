...
def benefit_statistics_get():
    ...
    stats = Benefit.objects.filter(is_deleted=False).aggregate(
        ...
        suspended_benefits=Count('id', filter=Q(status=BusinessStatusChoices.SUSPENDED))
    )
    
    # Add patient type breakdown
    patient_type_stats = Benefit.objects.filter(is_deleted=False).values(
        'in_or_out_patient'
    ).annotate(
        count=Count('id')
    ).order_by('-count')
...
def benefit_list_needing_attention():
    ...
    unused_benefits = Benefit.objects.filter(
        is_deleted=False,
        schemeitem__isnull=True
    )
    
    # Benefits with missing descriptions
    benefits_missing_description = Benefit.objects.filter(
        is_deleted=False
    ).filter(
...
def benefit_data_integrity_check():
    ...
    benefits_missing_data = Benefit.objects.filter(
        is_deleted=False
    ).filter(
        Q(benefit_name__isnull=True) | Q(benefit_name='') |
        Q(in_or_out_patient__isnull=True) | Q(in_or_out_patient='')
    )
    ...
    if benefits_missing_data.exists():
        issues.append({
            ...
            'benefits': list(benefits_missing_data.values_list('id', 'benefit_name'))
        })
    
    # Check for duplicate names with same patient type
    duplicate_combinations = Benefit.objects.filter(
        is_deleted=False
    ).values('benefit_name', 'in_or_out_patient').annotate(
        count=Count('id')
    ).filter(count__gt=1)
    ...
    if duplicate_combinations.exists():
        issues.append({
            ...
            'combinations': list(duplicate_combinations.values_list('benefit_name', 'in_or_out_patient'))
        })
    
    # Check for benefits with negative limit amounts
    negative_limits = Benefit.objects.filter(
        is_deleted=False,
        limit_amount__lt=0
    )
