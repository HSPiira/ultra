...
def scheme_item_list(*, filters: dict = None):
    ...
    """
    qs = SchemeItem.objects.select_related('scheme', 'content_type').filter(is_deleted=False)
...
def scheme_item_statistics_get():
    ...
    stats = SchemeItem.objects.filter(is_deleted=False).aggregate(
        ...
        inactive_scheme_items=Count('id', filter=Q(status=BusinessStatusChoices.INACTIVE)),
        suspended_scheme_items=Count('id', filter=Q(status=BusinessStatusChoices.SUSPENDED))
    )
    
    # Add content type breakdown
    content_type_stats = SchemeItem.objects.filter(is_deleted=False).values(
        'content_type__model'
    ).annotate(
        count=Count('id')
    ).order_by('-count')
    ...
    scheme_stats = SchemeItem.objects.filter(is_deleted=False).values(
        'scheme__scheme_name'
    ).annotate(
        count=Count('id')
    ).order_by('-count')
...
def scheme_item_list_needing_attention():
    ...
    inactive_scheme_items = SchemeItem.objects.filter(
        is_deleted=False,
        scheme__status=BusinessStatusChoices.INACTIVE
    )
    
    # Scheme items with no limit amount and no copayment
    items_no_limits = SchemeItem.objects.filter(
        is_deleted=False,
        limit_amount__isnull=True,
        copayment_percent__isnull=True
    )
    
    # Combine all
    all_attention_needed = suspended_items.union(
        inactive_scheme_items,
        items_no_limits
    )
...
def scheme_item_data_integrity_check():
    ...
    items_missing_data = SchemeItem.objects.filter(
        is_deleted=False
    ).filter(
        Q(scheme__isnull=True) |
        Q(content_type__isnull=True) |
        Q(object_id__isnull=True) | Q(object_id='')
    )
    ...
    if items_missing_data.exists():
        issues.append({
            ...
            'items': list(items_missing_data.values_list('id', 'scheme__scheme_name'))
        })
    
    # Check for duplicate scheme items
    duplicate_items = SchemeItem.objects.filter(
        is_deleted=False
    ).values('scheme', 'content_type', 'object_id').annotate(
        count=Count('id')
    ).filter(count__gt=1)
    ...
    if duplicate_items.exists():
        issues.append({
            ...
            'items': list(duplicate_items.values_list('scheme', 'content_type', 'object_id'))
        })
    
    # Check for scheme items with invalid copayment percentages
    invalid_copayment = SchemeItem.objects.filter(
        is_deleted=False
    ).filter(
    ...
    if invalid_copayment.exists():
        issues.append({
            ...
            'items': list(invalid_copayment.values_list('id', 'scheme__scheme_name'))
        })
    
    # Check for scheme items with negative limit amounts
    negative_limits = SchemeItem.objects.filter(
        is_deleted=False,
        limit_amount__lt=0
    )
