...
def scheme_statistics_get():
    ...
    company_stats = Scheme.objects.filter(is_deleted=False).values(
...
def scheme_list_needing_attention():
    ...
    schemes_no_items = Scheme.objects.filter(
        is_deleted=False,
        items__isnull=True
    )
    
    # Schemes with missing descriptions
    schemes_missing_description = Scheme.objects.filter(
        is_deleted=False
    ).filter(
...
def scheme_data_integrity_check():
    ...
    schemes_missing_data = Scheme.objects.filter(
        is_deleted=False
    ).filter(
        Q(scheme_name__isnull=True) | Q(scheme_name='') |
        Q(company__isnull=True) |
        Q(card_code__isnull=True) | Q(card_code='')
    )
    ...
    duplicate_codes = Scheme.objects.filter(
        is_deleted=False
    ).values('card_code').annotate(
        count=Count('id')
    ).filter(count__gt=1)
