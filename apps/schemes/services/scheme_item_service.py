...
class SchemeItemService:
    ...
    def scheme_item_create(*, scheme_item_data: dict, user=None):
        ...
        if scheme_item_data.get('limit_amount') is not None and scheme_item_data['limit_amount'] < 0:
    ...
    def scheme_item_update(*, scheme_item_id: str, update_data: dict, user=None):
        ...
        try:
            ...
            raise ValidationError("Scheme item not found")
        ...
        if 'scheme' in update_data or 'content_type' in update_data or 'object_id' in update_data:
            ...
            if qs.filter(
                ...
                raise ValidationError("Another scheme item with this combination already exists")
    ...
    def scheme_item_activate(*, scheme_item_id: str, user=None):
        ...
        scheme_item.save(update_fields=['status', 'is_deleted', 'deleted_at', 'deleted_by'])
        return scheme_item
    ...
    @staticmethod
    @transaction.atomic
    def scheme_items_bulk_status_update(*, scheme_item_ids: list, new_status: str, user=None):
        """
    ...
    def scheme_items_export_csv(*, filters: dict = None):
        ...
        # Write header
        writer.writerow([
            'ID', 'Scheme', 'Content Type', 'Object ID', 'Item Name',
            'Limit Amount', 'Copayment Percent', 'Status', 'Created At', 'Updated At'
        ])
