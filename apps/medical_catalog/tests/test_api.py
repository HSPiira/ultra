...
class MedicalCatalogAPITests(TestCase):
    ...
    def test_service_crud(self):
        res = self.client.post(reverse('service-list'), {"name": "Consult", "base_amount": 40, "service_type": "GEN"}, format='json')
        ...
        res = self.client.put(reverse('service-detail', args=[obj_id]), {"name": "Consult 2", "base_amount": 45, "service_type": "GEN"}, format='json')
        self.assertEqual(res.status_code, 200)
    ...
    def test_medicine_crud(self):
        res = self.client.post(reverse('medicine-list'), {"name": "Para", "dosage_form": "Tab", "unit_price": 2.5, "route": "Oral"}, format='json')
        self.assertEqual(res.status_code, 201)
        obj_id = res.data["id"]
        ...
        res = self.client.put(reverse('medicine-detail', args=[obj_id]), {"name": "Para 2", "dosage_form": "Tab", "unit_price": 3, "route": "Oral"}, format='json')
        self.assertEqual(res.status_code, 200)
    ...
    def test_labtest_crud(self):
        res = self.client.post(reverse('labtest-list'), {"name": "CBC", "base_amount": 15}, format='json')
        self.assertEqual(res.status_code, 201)
        obj_id = res.data["id"]
        ...
        res = self.client.put(reverse('labtest-detail', args=[obj_id]), {"name": "CBC+", "base_amount": 18}, format='json')
        self.assertEqual(res.status_code, 200)
    ...
    def test_hospital_item_price_crud(self):
        ...
        res = self.client.post(
            ...
            {"hospital": hospital.id, "content_type": 1, "object_id": service.id, "amount": 25, "available": True},
            format='json',
        )
        ...
        res = self.client.put(reverse('hospitalitemprice-detail', args=[obj_id]), {"hospital": hospital.id, "content_type": 1, "object_id": service.id, "amount": 27, "available": False}, format='json')
        self.assertEqual(res.status_code, 200)
