...
from apps.members.api.serializers import PersonSerializer, BulkPersonRowSerializer
from apps.members.selectors.person_selector import person_list, person_get
...
class PersonViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        person = PersonService.person_create(person_data=request.data, user=request.user)
        serializer = self.get_serializer(person)
        return Response(serializer.data, status=status.HTTP_201_CREATED)

    def update(self, request, *args, **kwargs):
        person = PersonService.person_update(person_id=kwargs['pk'], update_data=request.data, user=request.user)
        serializer = self.get_serializer(person)
